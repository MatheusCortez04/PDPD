...
get_disease_subgraph  = function(disease=c('MDD','BD')){
  ...
  vertices_in_lcc_global = extract_lcc_vertices(global_graph)
    
  disease_genes_df = get_disease_genes(disease)
  disease_genes_in_lcc = intersect(vertices_in_lcc_global, disease_genes_df$entrez_id)
...
plot_disease_module = function(disease=c('MDD','BD')){
  set.seed(10)
  disease = match.arg(disease)
  graph_name= c("MDD" = "Major Depressive Disorder: Disease Module Subgraph",
                "BD" ="Bipolar Disorder: Disease Module Subgraph"
  )
